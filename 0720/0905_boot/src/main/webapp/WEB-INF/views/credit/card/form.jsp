...
<html>
				...
				<table class="table table-hover">
					<tbody>
						...
						<tr>
							...
							<td>
								<p class="input-group">
									<select id="expiry_date_month" name="expiry_date_month" class="form-control">
									...
									<select id="expiry_date_year" name="expiry_date_year" class="form-control">
										<option value="0"> 선 택 </option>
								...
								<label for="expiry_date_month" id="expiry_date_month_label" class="write_label"></label>
							</td>
						</tr>
						<tr>
							<th> C V C </th>
						...
						<tr>
							<th> 비 밀 번 호 </th>
							<td>
								<input type="text" class="form-control" id="pwd" name="pwd" maxlength="2" placeholder="신용 카드 비밀번호 앞 두자리">
								<label for="pwd" id="pwd_label" class="write_label"></label>
							</td>
						</tr>
						<tr>
			...
			$.get(
					...
					, function(data, status) {
						if(data >= 1){
							...
							location.href="${pageContext.request.contextPath}/credit/card/form";
						} else {
	...
	$(document).ready(function() {
		$("#write_btn").click(function() {
			...
			if( $("#expiry_date_month").val() == 0
					|| $("#expiry_date_year").val() == 0 ){//허용되지 않은 글자는 null.
				$("#expiry_date_month_label").text("필수 입력 입니다.");
				return;
			} else { $("#expiry_date_month_label").text(""); }
			...
			if( $("#pwd").val().match(onlyNum) == null ){//허용되지 않은 글자는 null.
				$("#pwd_label").text("필수 입력 사항이며, 숫자만 허용 됩니다.");
				return;
			} else { $("#pwd_label").text(""); }

			$.post(
					"${pageContext.request.contextPath}/credit/card/insert"
					, {
						card_name : $("#card_name").val()
						, card_number : $("#card_number1").val()
										+ "-" + $("#card_number2").val()
										+ "-" + $("#card_number3").val()
										+ "-" + $("#card_number4").val()
