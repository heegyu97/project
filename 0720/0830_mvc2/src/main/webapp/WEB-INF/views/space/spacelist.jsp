								...
								>
									...
									<div id="div1" style = "width: 100% ; height: 100% ;">
										<c:forEach items="${list}" var="dto" begin = "${status.index}" end = "${status.index+5}" varStatus = "status2" >
											...
											<c:choose>
												<c:when test="${status2.index%6==0}">"position: absolute; left: 300px; top:100px; "</c:when>
												<c:when test="${status2.index%6==1}">"position: absolute; left: 220px; top:100px; "</c:when>
												<c:when test="${status2.index%6==2}">"position: absolute; left: 140px; top:100px; "</c:when>
												<c:when test="${status2.index%6==3}">"position: absolute; left: 300px; top:200px; "</c:when>
												<c:when test="${status2.index%6==4}">"position: absolute; left: 220px; top:200px; "</c:when>
												<c:when test="${status2.index%6==5}">"position: absolute; left: 140px; top:200px; "</c:when>
											</c:choose>
											>
