...
public class FileDownController {
	...
	public void fileDownload( String path, HttpServletResponse response ) throws IOException  {
		...
		//response: client로 전송되는 data집합.
		//header : http를 통해 전송하는 data packet에 추가되는 정도.
		...
		byte [] buffer = new byte[1024];//1024byte 단위로 파일을 읽기 위한 선언.
		int readDate = fis.read( buffer );
		while( readDate != -1 ) {//더이상 읽을 것이 없을 때 탈출. //EOF : end of file : 더 이상 읽을 데이터가 없다는 의미.
			sos.write(buffer);
			readDate = fis.read( buffer );
		}//while
